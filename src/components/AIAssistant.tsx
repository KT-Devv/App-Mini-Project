...
import { Send, Lightbulb, BookOpen, Calculator, Microscope, History, Sparkles, MessageCircle} from 'lucide-react';
...
const AIAssistant = () => {
  ...
  return (
    <div className="flex flex-col h-screen bg-gradient-to-br from-background to-muted/30">
      {/* Compact Header */}
      <div className="bg-card/95 backdrop-blur-sm border-b shadow-sm p-4 flex-shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-muted-foreground text-xs">Your intelligent learning companion</p>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            <Sparkles className="h-4 w-4 text-yellow-500" />
            <Badge variant="outline" className="bg-gradient-to-r from-blue-50 to-blue-100 dark:from-blue-950 dark:to-blue-900 text-blue-700 dark:text-blue-300 border-blue-200 dark:border-blue-800 text-xs">
              Premium AI
            </Badge>
          </div>
        </div>
      ...
      <div className="bg-card/90 backdrop-blur-sm border-b p-3 flex-shrink-0">
        ...
        <div className="grid grid-cols-2 gap-2">
          {quickPrompts.map((prompt, index) => (
            <Button
              ...
              variant="outline"
              size="sm"
              className="h-auto p-3 justify-start hover:shadow-md transition-all duration-200 group border-border hover:border-blue-300 dark:hover:border-blue-700"
          ...
          <div className="space-y-6 max-w-4xl mx-auto">
                ...
                <div
                  className={`max-w-[85%] shadow-lg ${
                    chat.type === 'user'
                      ...
                      : 'bg-card border border-border text-foreground rounded-3xl rounded-bl-lg'
                  }`}
                >
                  {chat.type === 'ai' && (
                    <div className="flex items-center space-x-3 p-5 pb-3 border-b border-border">
                      ...
                      <span className="text-base font-semibold text-blue-600 dark:text-blue-400">AI Assistant</span>
                      <Sparkles className="h-5 w-5 text-yellow-500" />
                    </div>
                  )}
                  <div className="p-6">
                    <p className="text-lg leading-relaxed font-medium">{chat.message}</p>
                    <p className={`text-sm mt-4 ${chat.type === 'user' ? 'text-blue-100' : 'text-muted-foreground'}`}>
                      {chat.time}
                    </p>
                  </div>
                </div>
            ...
            {isTyping && (
              <div className="flex justify-start animate-fade-in">
                <div className="bg-card border border-border rounded-3xl shadow-lg max-w-[85%]">
                  <div className="flex items-center space-x-3 p-5 pb-3 border-b border-border">
                    <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-blue-600 rounded-full flex items-center justify-center">
                      <img 
                        src="/uploads/e6eb7e5b-37be-4300-9bbb-ed1fcef6aa7e.png" 
                  ...
                  <div className="p-6">
                    <div className="flex items-center space-x-4">
                      ...
                      <span className="text-base text-muted-foreground font-medium">Thinking...</span>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div className="bg-card/95 backdrop-blur-sm border-t shadow-lg p-4 flex-shrink-0">
          ...
          <div className="flex space-x-3 mb-3">
            <div className="flex-1 relative">
              <Input
                ...
                className="rounded-2xl border-border focus:border-blue-500 focus:ring-blue-500 text-lg py-6 px-6 pr-16 h-auto bg-background"
              />
              <div className="absolute right-2 top-1/2 transform -translate-y-1/2">
                <Button 
                  onClick={handleSendMessage} 
                  size="sm" 
                  className="rounded-xl w-12 h-12 p-0 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 shadow-md"
